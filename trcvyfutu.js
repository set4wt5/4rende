...
const FILE_PATH = process.env.FILE_PATH || './tygytfy'; // 运行文件夹，节点文件存放目录
const projectPageURL = process.env.URL || '';        // 填写项目域名可开启自动访问保活，非标端口的前缀是http://
const intervalInseconds = process.env.TIME || 399;   // 自动访问间隔时间（120秒）
...
const NAME = process.env.NAME || 'thuytg';                     // 节点名称
const ARGO_PORT = process.env.ARGO_PORT || 65242;           // Argo端口，使用固定隧道token需和cf后台设置的端口对应
const PORT = process.env.SERVER_PORT || process.env.PORT || 32678; // 节点订阅端口，若无法订阅请手动改为分配的端口
